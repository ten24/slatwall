...
component  extends="Slatwall.meta.tests.unit.SlatwallUnitTestBase" hint="Tests that the crypto for comparing signatures for API are valid."
{
	...
	public void function encodeAndDecode_Base64_Test(){
		...
		addToDebug(variables.key);
		var result = variables.crypto.decode_Base64(variables.key);
		addToDebug(result);
		addToDebug("Decoding the signature: ");
		addToDebug("#variables.crypto.decode_Base64('ZDEyMzJhZmZhYzg4MTRmNzY3YjNjY2Y5YTRlMTQ1Zjg5MTQxMDE2NQ==')#");
	}
	...
	public void function checkTimestampIsNotExpired(){
		...
		addToDebug(variables.unixTime);

		addToDebug(variables.oldTimestamp);
		addToDebug(ABS(variables.unixTime - variables.oldTimestamp));
	...
	public void function createSignature_andTest(){
		...
		addToDebug("Client Signature is: " & signature);
