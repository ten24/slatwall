	...
	public any function getUncapturedPreAuthorizations( required any payment ) {
			...
			if(structKeyExists(thisData, "authorizationCode")) {
				...
				for(var a=1; a<=arrayLen(authorizations); a++) {
					if(thisData.authorizationCode eq authorizations[a].authorizationCode) {
						...
						authorizations[a].amountAuthorized = val(precisionEvaluate(authorizations[a].amountAuthorized + thisData.amountAuthorized));
						authorizations[a].amountReceived = val(precisionEvaluate(authorizations[a].amountReceived + thisData.amountReceived));
						thisDataAdded = true;
						break;
					}
				}
		...
		for(var a=1; a<=arrayLen(authorizations); a++) {
			if(authorizations[a].amountAuthorized gt authorizations[a].amountReceived) {
				sortedFound = false;
				authorizations[a].chargeableAmount = val(precisionEvaluate(authorizations[a].amountAuthorized - authorizations[a].amountReceived));
		...
		if(!isNull(arguments.order.getAccount()) && len(arguments.order.getAccount().setting('accountEligiblePaymentMethods'))) {
			...
			for(var i=1; i<=arrayLen(arguments.order.getOrderItems()); i++) {
				...
				for(var x=1; x<=listLen( epmList ); x++) {
					...
					paymentMethodMaxAmount[thisPaymentMethodID] = val(precisionEvaluate(paymentMethodMaxAmount[thisPaymentMethodID] + (arguments.order.getOrderItems()[i].getExtendedPriceAfterDiscount() + arguments.order.getOrderItems()[i].getTaxAmount())));
				}
			}
