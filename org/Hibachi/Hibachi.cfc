component extends="framework.one" {
	...
	variables.framework.beanFactoryOmitDirectoryAliases=false;
	...
	private void function setupBeanFactory() {
		var beanFactorySetupStartTime = getTickCount();
		...
		var coreBF = new framework.aop("/#variables.framework.applicationKey#/model", {
			...
			omitDirectoryAliases=#variables.framework.beanFactoryOmitDirectoryAliases#
		});
		...
		writeLog(file="#variables.framework.applicationKey#", text="General Log - Bean Factory Set in #round((getTickCount() - beanFactorySetupStartTime) / 100) / 10# seconds");
	}

	private void function reloadOrm() {
		// Reload ORM
		writeLog(file="#variables.framework.applicationKey#", text="General Log - ORMReload() started");
		var ormReloadStartTime = getTickCount();
		ormReload();
		writeLog(file="#variables.framework.applicationKey#", text="General Log - ORMReload() was successful. Execution time #round((getTickCount() - ormReloadStartTime) / 100) / 10# seconds");
	}
