...
component output="false" accessors="true" extends="HibachiProcess" {
	...
	public any function getExpenseLedgerAccount(){
		if(!structKeyExists(variables,'expenseLedgerAccount')){
			variables.expenseLedgerAccount = getService('LedgerAccountService').getLedgerAccount(getExpenseLedgerAccountID());
		}
		return variables.expenseLedgerAccount;
	}
	
	public array function getExpenseLedgerAccountIDOptions() {
		if(!structKeyExists(variables, "expenseLedgerAccountIDOptions")) {
			var expenseLedgerAccountIDs = getPhysical().setting('physicalEligibleExpenseLedgerAccount');
			if(!listLen(expenseLedgerAccountIDs)){
				expenseLedgerAccountIDs = getPhysical().setting('physicalDefaultExpenseLedgerAccount');
			}
			
			var ledgerAccountSmartList = getService('ledgerAccountService').getledgerAccountSmartList();
			ledgerAccountSmartList.addSelect('ledgerAccountID','value');
			ledgerAccountSmartList.addSelect('ledgerAccountName','name');
			ledgerAccountSmartList.addInFilter('ledgerAccountID',expenseLedgerAccountIDs);
			variables.expenseLedgerAccountIDOptions = ledgerAccountSmartList.getRecords();
		}
		
		return variables.expenseLedgerAccountIDOptions;
	}
