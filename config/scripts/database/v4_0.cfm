...
<!--- Update SwLocationConfiguration to set locationConfigurationCapacity to 1 and SwSku to set eventCapacity to 1 --->
<cftry>
	<cfquery name="local.updateData">
		UPDATE
			SwLocationConfiguration
		SET
			locationConfigurationCapacity = 1
		WHERE
			locationConfigurationCapacity IS NULL
	</cfquery>
	...
	<cfcatch>
		<cflog file="Slatwall" text="ERROR UPDATE SCRIPT - Update locationConfiguration to set locationConfigurationCapacity to 1 and SwSku to set eventCapacity to 1">
		<cfset local.scriptHasErrors = true />
	</cfcatch>
