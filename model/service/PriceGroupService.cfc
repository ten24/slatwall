...
component extends="HibachiService" persistent="false" accessors="true" output="false" {
	...
	public numeric function calculateSkuPriceBasedOnPriceGroupRate(required any sku, required any priceGroupRate) {
		...
		switch(arguments.priceGroupRate.getAmountType()) {
			case "percentageOff" :
				var newPrice = val(precisionEvaluate(arguments.sku.getPrice() - (arguments.sku.getPrice() * (arguments.priceGroupRate.getAmount() / 100))));
				...
				break;
			case "amountOff" :
				var newPrice = val(precisionEvaluate(arguments.sku.getPrice() - arguments.priceGroupRate.getAmount()));
				break;
			case "amount" :
	...
	public numeric function calculateSkuPriceBasedOnPriceGroupRateAndCurrencyCode(required any sku, required any priceGroupRate, required string currencyCode) {
		...
		switch(arguments.priceGroupRate.getAmountType()) {
			case "percentageOff" :
				var newPrice = val(precisionEvaluate(arguments.sku.getPriceByCurrencyCode(arguments.currencyCode) - (arguments.sku.getPriceByCurrencyCode(arguments.currencyCode) * (arguments.priceGroupRate.getAmount() / 100))));
				...
				break;
			case "amountOff" :
				var newPrice = val(precisionEvaluate(arguments.sku.getPriceByCurrencyCode(arguments.currencyCode) - arguments.priceGroupRate.getAmountByCurrencyCode(arguments.currencyCode)));
				break;
			case "amount" :
