				...
				<div class="card-body">
						...
						<cfif structKeyExists(option,'name') && structKeyExists(option,'value')>
							...
							<cfif len(trim(option['name'])) OR isFilterApplied>
			                	...
			                	<input 
			                		...
			                		>
			                	<label class="form-check-label" for="#arguments.title##option['value']#">#option['name']#</label>
							</cfif>
						</cfif>
						</div>
					</cfloop>
					...
					<cfif isArray(attributes.rangeData)>
					    ...
					    <form>
		                    ...
		                    <a id="apply#rangeSearchID#" href="##" class="btn btn-sm btn-block btn-secondary mt-3">Apply</a>
		                </form>
					</cfif>
