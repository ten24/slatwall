component extends="Slatwall.meta.tests.unit.SlatwallUnitTestBase" {
	...
	public void function promotionIntegrationAddAndRemove(){
		var orderData = {
			orderID = ''
		};
		var order = createTestEntity('order',orderData);
		...
		var order_removePromotionCode = {
			promotionCodeID = '8a804483472135b6014721625eee0123',
			orderID = order.getOrderID()
		};

		//Assert Promotion has been added
		order = variables.orderService.process(order,order_removePromotionCode,'removePromotionCode');
		assertEquals(arrayLen(order.getPromotionCodes()),0);

	}

	public void function promotionNoCodeIntegration(){
		var orderData = {
			orderID = ''
		};
		var order = createTestEntity('order',orderData);
