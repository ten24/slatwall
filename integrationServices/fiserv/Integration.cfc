...
component accessors="true" output="false" extends="Slatwall.integrationServices.BaseIntegration" implements="Slatwall.integrationServices.IntegrationInterface" {
	...
	public struct function getSettings() {
		var settings = {
			...
			apiSecret = {fieldType="password", encryptValue=true},
			storeID = {fieldType="text"},
			storeIDTemplate = {fieldType="text"},
			liveModeFlag = {fieldType="yesno", defaultValue="0"},
			debugModeFlag = {fieldType="yesno", defaultValue="0"},
			apiUrlLive = {fieldType="text", defaultValue="https://prod.api.firstdata.com/gateway/v2"},
			apiUrlTest = {fieldType="text", defaultValue="https://cert.api.firstdata.com/gateway/v2"}
		};
