...
component displayname="Collection" entityname="SlatwallCollection" table="SwCollection" persistent="true" hb_permission="this" accessors="true" extends="HibachiEntity" hb_serviceName="hibachiCollectionService" hb_processContexts="clone" {
	...
	property name="applyCollectionConfigGroupBys" type="boolean" persistent="false" default="0";
	...
	public any function init(){
		...
		variables.applyCollectionConfigGroupBys=false;
	...
	public boolean function isGroupBysAutoGenerated(){
		if( variables.applyCollectionConfigGroupBys && structKeyExists(getCollectionConfigStruct(),'groupBys') && listLen(getCollectionConfigStruct().groupBys) ){
			return false;
		}
		...
		if( structKeyExists(this.getCollectionConfigStruct(),'columns') ){
			for(var column in this.getCollectionConfigStruct().columns){
				...
				if( structKeyExists(column, "aggregate")  && 
					...
					( lCase(column.aggregate.aggregateFunction) != 'min' ||
					 lCase(column.aggregate.aggregateFunction) != 'max' )
				){
					return true;
				}
				
				if(ListLen(column.propertyIdentifier, '_') >= 2){
					 if(getService('hibachiService').hasToManyByEntityNameAndPropertyIdentifier(getCollectionObject(), convertAliasToPropertyIdentifier(column.propertyIdentifier))){
					 	return false;
					 }
				}
			}
	...
	public void function addGroupBy(required string groupByAlias){
		...
		if(ListContains(collectionConfig.groupBys,groupByList) == 0){
			...
			variables.applyCollectionConfigGroupBys = true;
		}
		this.setCollectionConfigStruct(collectionConfig);
	...
	public any function getGroupBys(){
		if(!structKeyExists(variables,'groupBys')){
			...
			//standard group by check
				...
				if(variables.applyCollectionConfigGroupBys){
					variables.groupBys = getCollectionConfigStruct().groupBys;
				}else if(isGroupBysAutoGenerated()){
					var groupByOverride="";
					var groupByList = "";
