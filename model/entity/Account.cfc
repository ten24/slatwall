...
component displayname="Account" entityname="SlatwallAccount" table="SwAccount" persistent="true" output="false" accessors="true" extends="HibachiEntity" cacheuse="transactional" hb_serviceName="accountService" hb_permission="this" hb_processContexts="addAccountLoyalty,addAccountPayment,createPassword,changePassword,clone,create,forgotPassword,lock,login,logout,resetPassword,setupInitialAdmin,unlock,updatePassword,generateAPIAccessKey" {
	...
	property name="accountAddressOptions" persistent="false"; 
	property name="accountPaymentMethodOptions" persistent="false"; 
	...
	public any function getAccountAddressOptions() {
		if(!structKeyExists(variables, 'accountPaymentMehodOptions')){
			...
			accountAddressCollectionList.setDisplayProperties('accountAddressName, address.streetAddress, address.city, address.stateCode, address.locality, address.country, address.postalCode, accountAddressID|value');
			...
			for(var accountAddress in accountAddresses){
				...
				var accountAddressOption = {
					"name":  addressName & accountAddress['address_country'],
					"value": accountAddress['value']  
				};
				arrayAppend(variables.accountPaymentMethodOptions, accountAddressOption);
			} 
		} 
