...
const ProductTypeListing = () => {
  ...
  if (!request.isFetching && !request.isLoaded) {
    setRequest({ ...request, isFetching: true, isLoaded: false, params: { urlTitle: slug }, makeRequest: true })
  }
  ...
  return (
    <Layout>
      { request.data.subProductTypes?.length &&
      ...
      { request.data.showProducts &&
        <ListingPage preFilter={{ productType_id: request.data.productTypeID }} hide={'productType'}>
          ...
          {/* <div className="container d-lg-flex justify-content-between py-2 py-lg-3">
            {request.isLoaded && <SWImage style={{ maxHeight: '150px', marginRight: '50px' }} customPath="/custom/assets/images/brand/logo/" src={request.data.imageFile?.length ? request.data.imageFile.split('/').reverse()[0] : ''} alt={request.data.title} />}
          </div> */}
