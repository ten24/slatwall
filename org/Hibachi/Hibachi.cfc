	...
	public void function setupRequest() {
		...
		if(!authorizationDetails.authorizedFlag) {
			...
			if(structKeyExists(url,'ng')){
				...
				var pageContextResponse = getPageContext().getResponse();
				...
				if(structKeyExists(authorizationDetails,'forbidden') && authorizationDetails.forbidden == true){
					pageContextResponse.setStatus(403, "#getSubsystem(request.context[ getAction() ])#:#hibachiConfig.loginDefaultSection#.#hibachiConfig.loginDefaultItem#");
					message['message'] = 'forbidden';
				}else if(structKeyExists(authorizationDetails,'timeout') && authorizationDetails.timeout == true){
					pageContextResponse.setStatus(401, "#getSubsystem(request.context[ getAction() ])#:#hibachiConfig.loginDefaultSection#.#hibachiConfig.loginDefaultItem#");
