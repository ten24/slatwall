...
<cfset test = ORMExecuteQuery("
	SELECT new Map( SUM(_product.calculatedQATS) as calculatedQATSSUM, DATE_FORMAT(_product.modifiedDateTime,'%Y-%m-%d') as modifiedDateTime) FROM SlatwallProduct as _product  GROUP BY  DATE_FORMAT(_product.modifiedDateTime,'%Y-%m-%d')",{},false,{maxresults=5})
/>
...
<html lang="en" id="ngApp" ng-strict-di>
	<head>
		...
		<script type="text/javascript" src="#request.slatwallScope.getBaseURL()#/org/Hibachi/HibachiAssets/js/jquery-3.3.1.min.js"></script>
		<script type="text/javascript" src="#request.slatwallScope.getBaseURL()#/org/Hibachi/HibachiAssets/js/jquery-migrate-1.4.1.min.js"></script>
