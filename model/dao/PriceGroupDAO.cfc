...
<cfcomponent extends="HibachiDAO">
	...
	<cffunction name="getPriceGroupRateBySkuID" returntype="any" access="public">
		...
		<cfset results = ormExecuteQuery("SELECT pgr FROM SlatwallPriceGroupRate pgr INNER JOIN FETCH pgr.skus pgrs WHERE pgr.priceGroup.activeFlag=:one AND pgr.priceGroup.priceGroupID=:priceGroupID AND pgrs.skuID=:skuID", {one=1,priceGroupID=arguments.priceGroupID, skuID=arguments.skuID}, false, {maxresults=1} ) />
	...
	<cffunction name="getPriceGroupRateByProductID" returntype="any" access="public">
		...
		<!--- The results should be unique but previous code made me unsure if that is true. --->
		<cfset results = ormExecuteQuery("SELECT pgr FROM SlatwallPriceGroupRate pgr INNER JOIN FETCH pgr.products pgrp WHERE pgr.priceGroup.activeFlag=:one AND pgr.priceGroup.priceGroupID=:priceGroupID AND pgrp.productID=:productID", {one=1,priceGroupID=arguments.priceGroupID,productID=arguments.productID}, false, {maxresults=1}) />
	...
	<cffunction name="getPriceGroupRateByProductTypeID" returntype="any" access="public">
		...
		<!--- The results should be unique but previous code made me unsure if that is true. --->
		<cfset results = ormExecuteQuery("SELECT pgr FROM SlatwallPriceGroupRate pgr INNER JOIN FETCH pgr.productTypes pgrpt WHERE pgr.priceGroup.activeFlag=:one AND pgr.priceGroup.priceGroupID=:priceGroupID AND pgrpt.productTypeID=:productTypeID", {one=1,priceGroupID=arguments.priceGroupID, productTypeID=arguments.productTypeID}, false, {maxresults=1}) />
	...
	<cffunction name="getGlobalPriceGroupRate" returntype="any" access="public">
		...
		<!--- The results should be unique but previous code made me unsure if that is true. --->
		<cfset results = ormExecuteQuery("SELECT pgr FROM SlatwallPriceGroupRate pgr WHERE pgr.priceGroup.activeFlag=:one AND pgr.priceGroup.priceGroupID=:priceGroupID AND pgr.globalFlag=1", {one=1,priceGroupID=arguments.priceGroupID}, false, {maxresults=1}) />
