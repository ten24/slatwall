component extends="FW1.framework" {
	...
	public void function setupGlobalRequest() {
		...
		if(!structKeyExists(request, "#variables.framework.applicationKey#Scope")) {
			...
			if(structKeyExists(request,'context')){
				//Unique Request Value to prevent duplicate form posts
				if(!getHibachiScope().getService("HibachiCacheService").hasCachedValue("previousURV")){
					getHibachiScope().getService("HibachiCacheService").setCachedValue("previousURV",[]);
				}
				if(structKeyExists(request.context, "urv")) {
					if(arrayFind(getHibachiScope().getService("HibachiCacheService").getCachedValue("previousURV"), request.context.urv)) {
						...
						arrayAppend(getHibachiScope().getService("HibachiCacheService").getCachedValue("previousURV"), request.context.urv);
						request.context.urvIsValid = true;
					}
				} else {
					//this variable is populated into a hidden input to prevent duplicate requests; recycles by form submission
					request.context.urv = createUUID();
				}
			}
	...
	public void function endHibachiLifecycle() {
		...
		//pop the valid request on completion
		if(structKeyExists(request,'context') && structKeyExists(request.context,'urvIsValid') && request.context.urvIsValid){
			arrayDeleteAt(
				getHibachiScope().getService("HibachiCacheService").getCachedValue("previousURV"),
				arrayFind(getHibachiScope().getService("HibachiCacheService").getCachedValue("previousURV"), request.context.urv)
			);
		}
