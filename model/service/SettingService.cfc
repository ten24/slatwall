...
component extends="HibachiService" output="false" accessors="true" {
	...
	public any function saveSetting(required any entity, struct data={}) {
		...
		if(!getHibachiScope().getORMHasErrors()) {
			...
			// If calculation is needed, then we should do it
			if(listFindNoCase("skuAllowBackorderFlag,skuAllowPreorderFlag,skuQATSIncludesQNROROFlag,skuQATSIncludesQNROVOFlag,skuQATSIncludesQNROSAFlag,skuTrackInventoryFlag,skuOrderMinimumQuantity,skuOrderMaximumQuantity", arguments.entity.getSettingName())) {
				updateStockCalculated();
			}
			//reset cache by site
			if(
