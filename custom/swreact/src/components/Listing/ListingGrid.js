        ...
        pageRecords.map(({ product_urlTitle, product_productID, product_productName, sku_imageFile, listPrice, sku_skuID }, index) => {
          return (
            <div key={sku_skuID + `${index}`} className="col-md-4 col-sm-6 px-2 mb-4">
              <ProductCard urlTitle={product_urlTitle} productID={product_productID} productName={product_productName} listPrice={listPrice} skuID={sku_skuID} defaultProductImageFiles={[sku_imageFile]} />
            </div>
          )
        })}
    </div>
