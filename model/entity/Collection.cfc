...
component displayname="Collection" entityname="SlatwallCollection" table="SwCollection" persistent="true" hb_permission="this" accessors="true" extends="HibachiEntity" hb_serviceName="collectionService" {
	...
	// Paging Methods
	public array function getPageRecords(boolean refresh=false, boolean noAuth=false) {
		try{
			var HQL = '';
			var HQLParams = {};
	...
	private any function getSelectionsHQL(required array columns, boolean isDistinct=false, boolean forExport=false, boolean noAuth=false){
		var isDistinctValue = '';
		...
		var columnsHQL = '';
		if(getHibachiScope().authenticateCollection('read', this) || arguments.noAuth){
		...
		if(!isNull(collectionConfig.baseEntityName)){
			...
			if(!isNull(collectionConfig.columns) && arrayLen(collectionConfig.columns) && arguments.excludeSelectAndOrderBy eq false){
				...
				if(!this.getNonPersistentColumn() && !len(this.getProcessContext())){
					selectHQL &= getSelectionsHQL(columns=collectionConfig.columns, isDistinct=isDistinct, forExport=arguments.forExport, noAuth=arguments.noAuth);
				}
