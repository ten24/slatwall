component accessors="true" persistent="false" output="false" extends="HibachiObject" {
	...
	public numeric function getRecordsCount() {
		if(!structKeyExists(variables, "recordsCount")) {
			if(getCacheable() && getService("hibachiCacheService").hasCachedValue(getCacheName()) && structKeyExists(getService("hibachiCacheService").getCachedValue(getCacheName()), "recordsCount")) {
				variables.recordsCount = getService("hibachiCacheService").getCachedValue(getCacheName()).recordsCount;
			} else {
				if(!structKeyExists(variables,"records")) {
					...
					if(getCacheable()) {
						var structToCache = {};
						structToCache.recordsCount = variables.recordsCount;
						getService("hibachiCacheService").setCachedValue(getCacheName(), structToCache);
					}
				} else {
	...
	private void function saveState() {
		...
		// Get the saved state struct if it exists
		if(getHibachiScope().hasSessionValue('smartListSavedState')) {
			states = getHibachiScope().getSessionValue('smartListSavedState');
		}
