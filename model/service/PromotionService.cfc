...
component extends="HibachiService" persistent="false" accessors="true" output="false" {
		...
		for(var orderItem in arguments.order.getOrderItems()) {
			...
			for(var key in salePriceDetails) {
				if(structKeyExists(salePriceDetails[key], "salePrice") && salePriceDetails[key].salePrice < orderItem.getSkuPrice()) {
					...
					var discountAmount = val(precisionEvaluate((orderItem.getSkuPrice() * orderItem.getQuantity()) - (salePriceDetails[key].salePrice * orderItem.getQuantity())));
					...
					if(arguments.promotionPeriodQualifications[arguments.promotionReward.getPromotionPeriod().getPromotionPeriodID()].orderItems[ orderItem.getOrderItemID() ]) {
						...
						if( getOrderItemInReward(arguments.promotionReward, orderItem) ) {
							...
							if( isNull(orderItem.getAppliedPriceGroup()) || arguments.promotionReward.hasEligiblePriceGroup( orderItem.getAppliedPriceGroup() ) ) {
								...
								var discountAmount = val(precisionEvaluate(originalDiscountAmount - (orderItem.getExtendedSkuPrice() - orderItem.getExtendedPrice())));
							...
							if(discountAmount > 0) {
								...
								var discountPerUseValue = val(precisionEvaluate(discountAmount / discountQuantity));
					...
					if(needToRemove < thisDiscountQuantity) {
						...
						for(var y=arrayLen(arguments.orderItemQualifiedDiscounts[ orderItemID ]); y>=1; y--) {
							if(arguments.orderItemQualifiedDiscounts[ orderItemID ][y].promotionRewardID == prID) {
								...
								arguments.orderItemQualifiedDiscounts[ orderItemID ][y].discountAmount = val(precisionEvaluate(precisionEvaluate(arguments.orderItemQualifiedDiscounts[ orderItemID ][y].discountAmount / thisDiscountQuantity) * precisionEvaluate(thisDiscountQuantity - needToRemove)));
	...
	private numeric function getDiscountAmount(required any reward, required numeric price, required numeric quantity, string currencyCode) {
		...
		var originalAmount = val(precisionEvaluate(arguments.price * arguments.quantity));

		if(structKeyExists(arguments, "currencyCode") && len(arguments.currencyCode)){
			switch(reward.getAmountType()) {
				case "percentageOff" :
					discountAmountPreRounding = val(precisionEvaluate(originalAmount * (reward.getAmountByCurrencyCode(arguments.currencyCode)/100)));
					break;
				case "amountOff" :
					discountAmountPreRounding = reward.getAmountByCurrencyCode(arguments.currencyCode) * quantity;
					break;
				case "amount" :
					discountAmountPreRounding = val(precisionEvaluate((arguments.price - reward.getAmountByCurrencyCode(arguments.currencyCode)) * arguments.quantity));
					break;
			}
			...
			switch(reward.getAmountType()) {
				case "percentageOff" :
					discountAmountPreRounding = val(precisionEvaluate(originalAmount * (reward.getAmount()/100)));
					break;
				case "amountOff" :
					discountAmountPreRounding = reward.getAmount() * quantity;
					break;
				case "amount" :
					discountAmountPreRounding = val(precisionEvaluate((arguments.price - reward.getAmount()) * arguments.quantity));
					break;
			}
		}

		if(!isNull(reward.getRoundingRule())) {
			roundedFinalAmount = getRoundingRuleService().roundValueByRoundingRule(value=val(precisionEvaluate(originalAmount - discountAmountPreRounding)), roundingRule=reward.getRoundingRule());
			discountAmount = val(precisionEvaluate(originalAmount - roundedFinalAmount));
		} else {
