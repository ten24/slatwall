...
module hibachi.pagination{
    ...
    export class PaginationService extends hibachi.core.BaseService{
        public static $inject = [
            'utilityService'
        ];
        ...
        public createPagination = ():Paginator =>{
            var uuid= this.utilityService.createID(10);
            this.paginations[uuid] = new Paginator(uuid);
            return this.paginations[uuid];
        };

        public getPagination=(uuid):Paginator =>{
            if(!uuid) return;
            return this.paginations[uuid];
        };
    }
