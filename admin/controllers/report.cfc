...
component output="false" accessors="true" extends="Slatwall.org.Hibachi.HibachiControllerEntity" {
	...
	this.secureMethods=listAppend(this.secureMethods,'widget');
		...
		if(structKeyExists(arguments.rc, "reportSite")) {
			salesRevenueThisPeriodCollectionList.addFilter('orderCreatedSite.siteID', 'NULL','IS');
		} else {
			salesRevenueThisPeriodCollectionList.addFilter('orderCreatedSite.siteID', arguments.rc.reportSite,'=');
		}
		if( structKeyExists(arguments.rc, "reportStartDateTime") &&  structKeyExists(arguments.rc, "reportEndDateTime")){
			salesRevenueThisPeriodCollectionList.addFilter('createdDateTime', arguments.rc.reportStartDateTime, '>=');
			salesRevenueThisPeriodCollectionList.addFilter('createdDateTime', arguments.rc.reportEndDateTime, '<=');			
		}
		...
		if(structKeyExists(arguments.rc, "reportSite")) {
			...
			accountsThisPeriodCollectionList.addFilter('accountCreatedSite.siteID', arguments.rc.reportSite,'=');
		}
		if( structKeyExists(arguments.rc, "reportStartDateTime") &&  structKeyExists(arguments.rc, "reportEndDateTime")){
			accountsThisPeriodCollectionList.addFilter('createdDateTime', arguments.rc.reportStartDateTime, '>=');
			accountsThisPeriodCollectionList.addFilter('createdDateTime', arguments.rc.reportEndDateTime, '<=');			
		}
		...
		public void function tomsreports(required struct rc) {
			arguments.rc.response = {};
	        arguments.rc['ajaxRequest'] = true;
	        arguments.rc.headers["Content-Type"] = 'application/json';

			var getPopularReports = getService('HibachiService').getCollectionList('Collection')
			getPopularReports.setDisplayProperties('collectionID,collectionName,collectionConfig,collectionCode,reportFlag,createdByAccountID')
			
			// just check for null accountOwner.accountID... 
			getPopularReports.addFilter('collectionCode','sw-default-report-%','like')
			getPopularReports.addFilter('reportFlag',1);
			var popularReportsRecords = getPopularReports.getRecords()
			for (popularReportsRecord in popularReportsRecords) {
			    popularReportsRecord.collectionConfig = deserializeJSON(popularReportsRecord.collectionConfig)
			}
			rc.response['popularReports'] = popularReportsRecords
			
			var getallReports = getService('HibachiService').getCollectionList('Collection')
			getallReports.setDisplayProperties('collectionID,collectionName,collectionConfig,collectionCode,reportFlag,createdByAccountID')
			getallReports.addFilter('reportFlag',1);
			var allReportRecords = getallReports.getRecords()
			for (allReportRecord in allReportRecords) {
			    allReportRecord.collectionConfig = deserializeJSON(allReportRecord.collectionConfig)
			}
			rc.response['allReports'] = allReportRecords


			var getMyCustomReports = getService('HibachiService').getCollectionList('Collection')
			getMyCustomReports.setDisplayProperties('collectionID,collectionName,collectionConfig,collectionCode,reportFlag,createdByAccountID')
			getMyCustomReports.addFilter('reportFlag',1);
			getMyCustomReports.addFilter('createdByAccountID', getHibachiScope().getAccount().getAccountID() ,'=', 'OR', '', 'group1');
			getMyCustomReports.addFilter('accountOwner.accountID', getHibachiScope().getAccount().getAccountID() ,'=','OR','', 'group1');
			var customRecords = getMyCustomReports.getRecords()

			for (customRecord in customRecords) {
			    customRecord.collectionConfig = deserializeJSON(customRecord.collectionConfig)
			}
			rc.response['myCustomReports'] = customRecords
