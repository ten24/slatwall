  ...
  return (
    <div className="row mx-n2">
      ...
      {!isFetching &&
        ...
        pageRecords.map(({ product_urlTitle, product_productID, skuPrice, product_productName, product_productCode, sku_imageFile, listPrice, sku_skuID }, index) => {
          return (
            <div key={sku_skuID + `${index}`} className="col-md-4 col-sm-6 mb-4 p-0">
              <ProductCard urlTitle={product_urlTitle} productID={product_productID} productName={product_productName} productCode={product_productCode} calculatedSalePrice={skuPrice} listPrice={listPrice} skuID={sku_skuID} imageFile={sku_imageFile} />
            </div>
          )
        })}
