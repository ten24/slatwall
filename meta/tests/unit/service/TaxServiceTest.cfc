...
component extends="Slatwall.meta.tests.unit.SlatwallUnitTestBase" {
	...
	public void function addTaxAddressesStructBillingAddressKey_sets_struct_billingAddress_key_from_order_billingAddress(){
		var accountAddress = request.slatwallScope.getService("accountService").newAccountAddress();
		var newOrder = request.slatwallScope.newEntity('Order').setBillingAddress( accountAddress );
		var taxAddressesStruct = variables.service.addTaxAddressesStructBillingAddressKey( newOrder );
		assert(structKeyExists(taxAddressesStruct, 'taxBillingAddress'));
	}
	
	public void function addTaxAddressesStructBillingAddressKey_sets_struct_billingAddress_key_from_order_orderPayments(){
		var newOrder = request.slatwallScope.newEntity('Order').getService("orderService").newOrderPayment();
		var taxAddressesStruct = variables.service.addTaxAddressesStructBillingAddressKey(newOrder);
		assert(structKeyExists(taxAddressesStruct, 'taxBillingAddress'));
	}
}