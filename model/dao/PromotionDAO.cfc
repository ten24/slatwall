	...
	<cffunction name = "getOrderItemSalePricePromotionRewardsQuery">
		...
		<cfloop query="orderItemDiscountsQuery">
			...
			<cfelseif orderItemDiscountsQuery.salePriceDiscountType EQ "percentageOff" >
				<cfset querySetCell(orderItemDiscountsQuery, "salePrice", precisionEvaluate(orderItemDiscountsQuery.originalPrice - (round( orderItemDiscountsQuery.originalPrice * (orderItemDiscountsQuery.amount / 100) * 100 )/100) ), orderItemDiscountsQuery.currentRow )>
			</cfif>
		</cfloop>
		...
		<cfquery name="allDiscountsQuery">
		  ...
		  UNION
			...
			<cfif structKeyExists(arguments, "currencyCode") && len(arguments.currencyCode)>
				...
				LEFT JOIN SwPromotionRewardCurrency AS prCurrency
