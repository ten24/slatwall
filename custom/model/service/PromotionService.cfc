component extends="Slatwall.model.service.PromotionService" {
	...
	public any function duplicatePromotionReward(required any promotionReward){
		var newPromotionReward = super.duplicatePromotionReward( argumentCollection=arguments );
		
		for(var customPriceField in variables.customPriceFields){
			var customAmount = arguments.promotionReward.invokeMethod('get#customPriceField#Amount');
			if(!isNull( customAmount )){
				newPromotionReward.invokeMethod('set#customPriceField#Amount',{1=customAmount});
			}
		}
